...
class Transport(AMQPTransport):
    ...
    def establish_connection(self):
        ...
        print 'conninfo: %s' % conninfo
       
        print 'self.default_connection_params: %s' % self.default_connection_params.items()
        ...
        print hosts
        print 'len(hosts): %s' % len(hosts)
        random.shuffle(hosts)
        print 'len(hosts) after shuffle: %s' % len(hosts)

        while True:
            #print 'len(hosts) in while: %s' % len(hosts)
            if len(hosts) == 0:
            ...
            print 'host: %s' % which_host
            print 'userid: %s' % conninfo.userid
            print 'password: %s' % conninfo.password
            ...
            try:
                ...
                print "Connection error when connecting to broker: %s" % e
                continue
